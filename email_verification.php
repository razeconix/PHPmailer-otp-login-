	...
	if($no_of_row > 0)
	{
		...
		foreach($result as $row)
		{
			if($row['user_email_status'] == 'not verified')
			{
				...
				if(isset($sub_result))
				{
					$message = '<label class="text-success">ยืนยันตัวตนผ่าน Email เรียบร้อย <br />เข้าสู่ระบบที่นี่ - <a href="login.php">Login</a></label>';
				}
			}
			else
			{
				$message = '<label class="text-info">คุณได้ทำการยืนยันตัวตนเรียบร้อยแล้ว <br />เข้าสู่ระบบที่นี่ - <a href="login.php">Login</a></label>';
			}
		}
	}
	else
